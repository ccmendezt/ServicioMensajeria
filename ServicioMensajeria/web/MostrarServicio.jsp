...
<%
    ResultSet servicio = (ResultSet)session.getAttribute("servicioFiltro"); 
%>
<!DOCTYPE html>
<html>
    <head>
        ...
        <link rel="stylesheet" href="css/mostrarServicio.css"/>
    </head>
    <body>
        <h1 align="center">Detalle servicio</h1>
        <table border="1" class="table table-dark">
            <thead class="thead-dark">
                <tr>
                    <th>Id servicio</th><th>Dia</th><th>Hora</th><th>Tipo de servicio</th><th>Costo</th><th>Fecha cambio estado</th><th>Comentario cambio estado</th><th>Medio de pago</th><th>Id ciudad</th><th>Tipo documento cliente</th><th>Numero documento cliente</th><th>Calificacion</th><th>Tipo paquete</th><th>Tipo documento mensajero</th><th>Numero documento mensajero</th>
                </tr>
            </thead>
            <% while (servicio.next()) {%>
            <tr>
                <td><%= servicio.getInt("k_idservicio")%></td>
                ...
                <td><%= servicio.getString("f_cambioestado")%></td>
                <td><%= servicio.getString("n_cmtcambioestado")%></td>
                <td><%= servicio.getString("i_mediopago")%></td>
                <td><%= servicio.getString("k_idCiudad")%></td>
                <td><%= servicio.getString("k_tipodoccli")%></td>
                <td><%= servicio.getString("k_nrodoccli")%></td>
                <td><%= servicio.getString("q_calificacion")%></td>
                <td><%= servicio.getString("k_idtipopaquete")%></td>
                <td><%= servicio.getString("k_tipodocmen")%></td>
                <td><%= servicio.getString("k_nrodocmen")%></td>
            </tr>
            <% }%>
        </table>
        
        <div id="centrar">
            <button class="btn btn-primary" <button onclick="window.location.href = 'InicioSesionCliente.jsp'">Inicio</button>
        </div>
